...
\subsection{\textit{Framework} para desarrollo web}
    ...
    muy rápido y limpio. Tiene una arquitectura MTV (Model-Template-View), donde el View
    sería el equivalente a Controller en MVC. El esquema básico de este patrón es el
    siguiente:
    ...
    modelo MVC o MVT, sino que permite el desarrollo del front-end y el back-end de forma
...
El resto de plantillas de este diseño son las que se describen en las secciones siguientes.
    ...
    diseño de esta tiene relación con un componente de bootstrap llamado \textbf{Carousel}, que
    ...
    el código, este incorpora un borde naranja al componente que lo posea.
...
estos dos últimos almacenados siempre en \textbf{variables de entorno} por temas de
seguridad.\\
...
Esta sería la interfaz gráfica de la API, donde tenemos una pestaña de \textbf{login} para
...
Procedamos a explicar la implementación necesaria para esta página de logs:
    ...
    \end{minted}
        ...
        definido las siguientes partes:
            ...
            \begin{itemize}
                ...
                Tenemos que prevenir que ante una posible caída de la aplicación los logs
                se pierdan, por lo que estos se almacenarán en la carpeta \textbf{/var/log/}
                y el archivo que los contendrá será \textbf{myFindings.log}. Esta sería la
                ruta por defecto, aunque también podría modificarse a través de una
                variable de entorno. Quedando el manejador con la siguiente configuración:
    ...
    \end{minted}
                ...
                \item \textbf{Logger}: finalmente, queda la configuración del logger
                que recogerá los logs que proceden desde el archivo views.py de la
                aplicación. \\

                Como hemos mencionado anterioremente, el logger de las vistas se
                identifica con el nombre \textbf{myFindings.views}, por lo que este
                será el nombre de nuestro logger. Además, usará el handler especificado
                en el punto anterior y sólo controlará mensajes que tengan un nivel de log
                (\textbf{log level}) INFO o superior. Su configuración quedaría de la
                siguiente forma:
    ...
    \end{minted}
        ...
        \item Finalmente, deberemos crear la ruta correspondiente para la vista de logs
        que podrá ver un arqueólogo administrador. Para ello, definimos la correspondiente
        ruta en el archivo \textbf{urls.py}, creamos el template (HTML) correspondiente
        que se renderizará, y la vista que manejará la petición. Dicha vista hará uso
        de la función \href{https://docs.python.org/3/library/functions.html#open}{open}
        de Python para abrir el archivo de logs y procesarlo línea por línea, creando una
        lista que luego será enviada como un parámetro al template. Un ejemplo de la vista
        de logs es el siguiente:
...
Dicho esto, procedamos a explicar el proceso de despliegue de nuestra aplicación.
